...
set "TOPIC=gcs.dmarc-events"
set "SUB=sub-ai-scorer"
...
if "%DLQ_TOPIC%"=="" set "DLQ_TOPIC=dlq.%SUB%"
...
gcloud services enable run.googleapis.com pubsub.googleapis.com storage.googleapis.com cloudbuild.googleapis.com --project "%PROJECT_ID%"
REM (Optional) Force-create Pub/Sub service identity if it hasn't appeared yet
gcloud beta services identity create --service=pubsub.googleapis.com --project "%PROJECT_ID%" >nul 2>&1

REM 1) Create service accounts
echo -> Creating service accounts
gcloud iam service-accounts create "%RUNTIME_SA_NAME%" --project "%PROJECT_ID%" --display-name "AI Agent Runtime SA" >nul 2>&1
gcloud iam service-accounts create "%PUSH_SA_NAME%"    --project "%PROJECT_ID%" --display-name "Pub/Sub Push Invoker SA" >nul 2>&1
...
if /I "%ENV_REQUIRE_JWT%"=="true" (
  ...
  gcloud run services update "%SERVICE" --region "%REGION%" --project "%PROJECT_ID%" --update-env-vars REQUIRE_JWT=true,PUBSUB_ALLOWED_AUDIENCE=%SERVICE_URL%
)

REM 9) Pub/Sub topic + DLQ
echo -> Creating Pub/Sub topic %TOPIC% and DLQ %DLQ_TOPIC%
gcloud pubsub topics create "%TOPIC%"    --project "%PROJECT_ID%" >nul 2>&1
gcloud pubsub topics create "%DLQ_TOPIC%" --project "%PROJECT_ID%" >nul 2>&1

REM 10) Allow service agent to publish to DLQ
gcloud pubsub topics add-iam-policy-binding "%DLQ_TOPIC%" --member="serviceAccount:%PUBSUB_SERVICE_AGENT%" --role="roles/pubsub.publisher" --project "%PROJECT_ID%"
...
gcloud pubsub subscriptions add-iam-policy-binding "%SUB%" --member="serviceAccount:%PUBSUB_SERVICE_AGENT%" --role="roles/pubsub.subscriber" --project "%PROJECT_ID%"
...
gcloud pubsub topics add-iam-policy-binding "%TOPIC%" --member="serviceAccount:%GCS_SERVICE_AGENT%" --role="roles/pubsub.publisher" --project "%PROJECT_ID%" >nul 2>&1
...
if "%OBJECT_PREFIX%"=="" (
  gcloud storage buckets notifications create "gs://%BUCKET%" --topic="%TOPIC%" --event-types=OBJECT_FINALIZE --payload-format=json --project="%PROJECT_ID%"
) else (
  gcloud storage buckets notifications create "gs://%BUCKET%" --topic="%TOPIC%" --event-types=OBJECT_FINALIZE --payload-format=json --object-prefix="%OBJECT_PREFIX%" --project="%PROJECT_ID%"
)
